...
describe("The tag helper", () => {
  ...
  describe("name tag", () => {
    ...
    it("should return the fallback prop if name tag is not found", () => {
      ...
      expect(tagHelper.getEntityName(entityWithoutTagNameButFallback, "fallback")).toBe(
        "Testfallback",
      );
    });
    ...
    it("should return the fallback if name tag does not exist and no fallback", () => {
      ...
      expect(tagHelper.getEntityName(entityWithoutTagNameAndNoFallback)).toBe(
        tagHelper.TAG_NAME_FALLBACK_VALUE,
      );
    });
    ...
    it("should return the fallback if name tag does not exist and undefined fallback", () => {
      ...
      expect(
        tagHelper.getEntityName(entityWithoutTagNameAndNoFallback, "nonExistantFallback"),
      ).toBe(tagHelper.TAG_NAME_FALLBACK_VALUE);
    });
  });
  ...
  describe("description tag", () => {
    ...
    it("should return the fallback prop if description tag is not found", () => {
      ...
      expect(tagHelper.getEntityName(entityWithoutTagDescriptionButFallback, "fallback")).toBe(
        "Testfallback",
      );
    });
    ...
    it("should return the fallback if description tag does not exist and no fallback", () => {
      ...
      expect(tagHelper.getEntityDescription(entityWithoutTagDescriptionAndNoFallback)).toBe(
        tagHelper.TAG_DESCRIPTION_FALLBACK_VALUE,
      );
    });
    ...
    it("should return the fallback if description tag does not exist and undefined fallback", () => {
      ...
      expect(
        tagHelper.getEntityDescription(
          entityWithoutTagDescriptionAndNoFallback,
          "nonExistantFallback",
        ),
      ).toBe(tagHelper.TAG_DESCRIPTION_FALLBACK_VALUE);
    });
  });
  ...
  describe("parsing string to Tag", () => {
    ...
    it("should allow for spaces in tag key", () => {
      expect(JSON.stringify(tagHelper.parseStringToTag("testing space in key:tagValue"))).toBe(
        JSON.stringify({
    ...
    it("should allow for spaces in tag value", () => {
      expect(
        JSON.stringify(tagHelper.parseStringToTag("tagKey:testing with space in tagValue")),
      ).toBe(
        JSON.stringify({
          key: "tagKey",
  ...
  describe("parse Tag to string", () => {
    it("should correctly parse a Tag to string format", () => {
      const tag: Tag = { key: "tagKey", value: "tagValue" };
